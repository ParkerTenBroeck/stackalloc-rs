...
fn unknown_size_iter() {
    ...
    assert_eq!(
        result,
        super::stackalloc_with_iter(10, iter, |buf| buf.iter().copied().sum::<u64>())
    );
}

fn unknown_size_iter_len() {
    assert_eq!(
        super::stackalloc_with_iter(1024, 0..100, |buf| buf.len()),
        100
    );
}
...
#[test]
fn exact_size_iter() {
    let iter = vec![1, 2, 13, 24, 100];
    let len = iter.len();
    let result: u64 = iter.iter().copied().sum();

    assert_eq!(
        super::stackalloc_from_iter_exact(iter, |buf| {
...
fn with_alloca() {
    ...
    let sum = super::alloca(SIZE, |buf| {
        ...
        eprintln!("Buffer is now {:?}", unsafe {
            std::mem::transmute::<_, &&mut [u8]>(&buf)
        });

        buf.iter()
            .map(|x| unsafe { x.assume_init() } as u64)
            .sum::<u64>()
    });
...
fn primitive_type() {
    assert_eq!(
        super::stackalloc(10, 12.0, |floats| {
...
mod bench {
    ...
    fn stackalloc_of_zeroed_bytes_unknown(b: &mut Bencher) {
        ...
        b.iter(|| {
            black_box(crate::alloca_zeroed(size, |b| {
                black_box(b);
            }));
        })
    }
